...
export function VideoPlayer({ src, poster }: VideoPlayerProps) {
  ...
  useEffect(() => {
    ...
    // Solo agregar el listener de teclado si el reproductor tiene el foco
    const keyListener = (e: KeyboardEvent) => {
      const active = document.activeElement;
      // Ignorar si el foco está en un input, textarea o contenteditable
      if (
        active && (
      ...
      // Solo responder si el foco está en el contenedor del reproductor
  ...
  return (
    <div
      ref={containerRef}
      className={`relative bg-transparent overflow-hidden group flex items-center justify-center transition-all duration-300 ${
    ...
    >
      ...
      <video
        ...
        className={`w-full h-full object-contain transition-all duration-300 ${
          isFullscreen ? 'max-w-screen max-h-screen' : 'max-w-[640px] max-h-[480px]'
        }`}
          ...
          <div className="relative group flex items-center gap-2">
            ...
            <div 
              className={`w-20 transition-all duration-200 ${
                showControls ? "opacity-100 w-20" : "opacity-0 w-0"
              }`}
            >
              <div className="relative w-full px-1">
                <input
                  ...
                  className="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer 
                    ...
                    [&::-moz-range-thumb]:hover:scale-125 [&::-moz-range-thumb]:hover:bg-purple-500"
                  style={{
